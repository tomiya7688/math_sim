...
def build_pathfinding_page(app: tk.Misc, parent: tk.Widget) -> tk.Frame:
    ...
    def read_params() -> dict:
        ...
        obstacles = float(obstacle_var.get())
        min_cost = float(min_cost_var.get())
        max_cost = float(max_cost_var.get())
        seed = int(seed_var.get())
        if width < 2 or height < 2 or not (0 <= obstacles < 1) or min_cost <= 0 or max_cost < min_cost:
            raise ValueError
        return dict(width=width, height=height, obstacle_probability=obstacles,
                    min_cost=min_cost, max_cost=max_cost, seed=seed)

    def finish(result: dict) -> None:
        state["result"] = result
        found = bool(result.get("found"))
        cost = float(result.get("cost", 0.0))
        visited = int(result.get("visited", 0))
        path = result.get("path", [])
        algorithm = str(result.get("algorithm", ""))
        summary_var.set(
            f"{algorithm} · {'found' if found else 'no path'} · cost {cost:.2f} · visited {visited} · steps {max(len(path)-1, 0)}"
        )
    ...
    def worker(params: dict, algorithm: str) -> None:
        try:
            result = solve_random_map(**params, algorithm=algorithm)
    ...
    def start() -> None:
        ...
        algorithm = ALGORITHM_LABELS[algorithm_var.get()]
        threading.Thread(target=worker, args=(params, algorithm), daemon=True).start()
